...
final class Controller {
    ...
    private Command[] _commands = {
        ...
        new Command("(?i)([a-j](?:[1-9]|10))\\-[a-j](?:[1-9]|10)"
                + "\\(([a-j](?:[1-9]|10))\\)", this::doMove),
        new Command("(?i)([a-j](?:[1-9]|10))\\s+([a-j](?:[1-9]|10))"
                + "\\s+([a-j](?:[1-9]|10))", this::doMove),
        new Command("new$", this::doNew),
        new Command("auto\\s+(?i)(black|white)(?-i)$", this::doAuto),
        new Command("manual\\s+(?i)(black|white)(?-i)$", this::doManual)
    };
    ...
    private void doAuto(Matcher auto) {
        String piece = auto.group(1);
        if (piece.toUpperCase().equals("WHITE")) {
            _white = _autoPlayerTemplate.create(WHITE, this);
        } else if (piece.toUpperCase().equals("BLACK")) {
            _black = _autoPlayerTemplate.create(BLACK, this);
        }
    }
    /** Command "Manual color".
     * @param manual Color */
    private void doManual(Matcher manual) {
        String piece = manual.group(1);
        if (piece.toUpperCase().equals("WHITE")) {
            _white = _manualPlayerTemplate.create(WHITE, this);
        } else if (piece.toUpperCase().equals("BLACK")) {
